...
class TreeViewCommand (
	...
	override def equals(obj: Any): Boolean = {
		if (!obj.isInstanceOf[TreeViewCommand]) {
			...
			return (title == that.title) &&
					...
					Option(arguments).map(_.zipWithIndex.map( { case (arg, idx) => arg == that.arguments(idx) }).reduce(_ && _)).getOrElse(false)
		}
	}
	...
	override def hashCode(): Int = {
		...
		hash = 31 * hash + (if (Option(arguments).isEmpty) 0 else arguments.hashCode)
		hash
	}
