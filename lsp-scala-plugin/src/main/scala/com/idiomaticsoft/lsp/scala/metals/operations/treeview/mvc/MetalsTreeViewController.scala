...
trait MetalsTreeViewController {
  ...
  def parentNodesMap
      : scala.collection.mutable.HashMap[String, Array[TreeViewNode]]
...
class MetalsTreeViewControllerImpl extends MetalsTreeViewController {
  ...
  val parentNodesMap =
    new scala.collection.mutable.HashMap[String, Array[TreeViewNode]]
  val uriTochildNodes =
    new scala.collection.mutable.HashMap[String, TreeViewNode]
  ...
  override def parentNodes(viewId: String): List[TreeViewNode] =
    parentNodesMap(viewId).toList

  private def createUriChain(
      node: TreeViewNode,
      list: List[String]
  ): List[String] = {
  ...
  private def addOrReplaceNodeIfNotThere(
      l: List[TreeViewNode],
      node: TreeViewNode
  ): List[TreeViewNode] = {
    l match {
  ...
  override def elementCollapsed(element: Any, collapsed: Boolean): Unit = {
    ScalaLSPPlugin()
      ...
      .treeViewNodeCollapseDidChange(
        new TreeViewNodeCollapseDidChangeParams(
  ...
  def getChildren(treeViewNode: TreeViewNode) = {
    val children = Option(languageServer)
      .map(
        _.treeViewChildren(
          new TreeViewChildrenParams(
  ...
  def visitPath(
      viewId: String,
      uriChain: List[String],
      elements: List[TreeViewNode],
      someParent: Option[TreeViewNode],
      f: TreeViewNode => Unit
  ): Option[TreeViewNode] = {
    uriChain match {
      case h :: tail =>
  ...
  override def reveal(
    ...
    if (Option(result).isDefined) {
      ...
      languageServer().treeViewVisibilityDidChange(
        new TreeViewVisibilityDidChangeParams("metalsBuild", true)
      )
      Display
        .getCurrent()
        .timerExec(
          1,
          () => {
            val foundNode = visitPath(
              "metalsBuild",
              result.getUriChain().toList.reverse,
              parentNodes("metalsBuild"),
              None,
              expandTreeViewNode("metalsBuild", _)
            )
            foundNode.map(
              y =>
                view
                  .viewer("metalsBuild")
                  .map(_.setSelection(new StructuredSelection(y), true))
            )
          }
        )
    }
