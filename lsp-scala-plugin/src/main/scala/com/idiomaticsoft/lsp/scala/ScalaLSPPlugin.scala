...
class ScalaLSPPlugin extends AbstractUIPlugin {
  ...
  def launchJob(): Unit = {
		...
		val vmParams = "-XX:+UseG1GC -XX:+UseStringDeduplication -Xss4m -Xms100m -Xmx2G -Dmetals.http=true -Dmetals.status-bar=on -Dmetals.icons=unicode -Dmetals.slow-task=on"
