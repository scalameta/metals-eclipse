  ...
  override def getParent(x: Any): Object = {
    (for {
      ...
      (for {
        ...
        val treeViewParentParams = new TreeViewChildrenParams(
          x.asInstanceOf[TreeViewNode].getViewId(),
          nodeUri
        )
        val treeViewNode = ls.treeViewParent(treeViewParentParams).get()
        (for (treeNodeUri <- Option(treeViewNode.getUri()))
          yield controller.uriTochildNodes(treeNodeUri))
          .getOrElse(controller.childToParent(nodeUri))
      }).getOrElse {
        null
      }
    }).getOrElse(null)
